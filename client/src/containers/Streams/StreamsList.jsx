...
import { uriConsumerGroupDelete, uriConsumerGroups } from '../../utils/endpoints';
...
import { getPageNumber, handlePageChange } from '../../utils/pagination';
import { Checkbox } from '@material-ui/core';

class StreamsList extends Root {
  state = {
    consumerGroups: [],
    ...
    if (response.results) {
      ...
      this.setState({ selectedCluster, totalPageNumber: response.page }, () =>
        this.props.history.push({
          pathname: `/ui/${this.state.selectedCluster}/streams`,
  ...
  handleConsumerGroup(consumerGroup) {
    ...
    consumerGroup
      // .filter(consumerGroup => consumerGroup.partitionAssignor === 'stream')
  ...
  render() {
    ...
    return (
      <div>
        <Header title="Stream Applications" history={history} />
        <nav
        ...
        <Table
          ...
          noContent={'Well fuck me sideways no data'}
        />
